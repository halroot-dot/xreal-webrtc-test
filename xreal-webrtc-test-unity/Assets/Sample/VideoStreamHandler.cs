...
public class VideoStreamHandler
{
    ...
    {
        Debug.Log($"[XREAL_WEBRTC][Handler] Creating VideoStreamHandler {width}x{height}");
        ...
        try
        {
            ...
            Debug.Log($"[XREAL_WEBRTC][Handler] RenderTexture created: {targetTexture.width}x{targetTexture.height}");
        ...
        {
            Debug.LogError($"[XREAL_WEBRTC][Handler] Initialization error: {e.Message}\n{e.StackTrace}");
    ...
    private void InitializeAndroidMediaPlayer(int width, int height)
    {
        ...
        Debug.Log("[XREAL_WEBRTC][Handler] MediaPlayer initialized successfully");
    }
    ...
    private void OnTextureUpdate()
    {
        ...
        {
            Debug.LogError($"[XREAL_WEBRTC][Handler] Texture update error: {e.Message}");
        }
    }
        ...
        lock (mediaPlayerLock)
        {
            try
            {
                if (mediaPlayer != null)
                {
                    ...
                    Debug.Log($"[XREAL_WEBRTC][Handler] Video loaded: {path}");
                }
            }
            catch (Exception e)
            {
                Debug.LogError($"[XREAL_WEBRTC][Handler] Failed to load video: {e.Message}");
            }
        }
    }
        ...
        if (mediaPlayer != null)
        {
            if (isMediaPlayerPrepared)
            {
                try
                {
                    ...
                    isPlaying = true;
                    isPendingPlay = false;
                    Debug.Log("[XREAL_WEBRTC][Handler] MediaPlayer started");
                }
                catch (Exception e)
                {
                    Debug.LogError($"[XREAL_WEBRTC][Handler] Failed to start MediaPlayer: {e.Message}");
                }
            }
            else
            {
                isPendingPlay = true;
                Debug.Log("[XREAL_WEBRTC][Handler] Play pending until prepared");
            }
        }
    }
    ...
    public void Stop()
    {
        if (mediaPlayer != null && isPlaying)
        {
            try
            {
                ...
                isPlaying = false;
                isPendingPlay = false;
                Debug.Log("[XREAL_WEBRTC][Handler] MediaPlayer stopped");
            }
            catch (Exception e)
            {
                Debug.LogError($"[XREAL_WEBRTC][Handler] Failed to stop MediaPlayer: {e.Message}");
            }
        }
    }
    ...
    public void Release()
    {
        ...
        lock (mediaPlayerLock)
        {
            if (mediaPlayer != null)
            {
                ...
                {
                    Debug.LogError($"[XREAL_WEBRTC][Handler] Error releasing MediaPlayer: {e.Message}");
                }
                finally
                {
                    mediaPlayer = null;
            ...
            if (surfaceTexture != null)
            {
                ...
                {
                    Debug.LogError($"[XREAL_WEBRTC][Handler] Error releasing SurfaceTexture: {e.Message}");
                }
                finally
                {
                    surfaceTexture = null;
